...
export interface SecurityConfig {
  ...
  queryTimeoutMs: number;       // 查询超时时间(毫秒)
  enableStrictMode: boolean;    // 严格模式
  allowedTables?: string[];     // 允许访问的表(可选)
  blockedTables?: string[];     // 禁止访问的表(可选)
}
 ...
 * 检测SQL注入威胁
 */
export function detectSqlInjection(sql: string): SecurityCheckResult {
...
export function validateOperation(
  ...
  // 检查是否以危险操作开头
  const startsWithDangerous = dangerousOperations.some(op =>
 ...
 * 强制添加结果集限制
 */
export function enforceResultLimit(sql: string, maxRows: number): string {
  const normalizedSql = sql.trim().toUpperCase();
...
export function validateTableAccess(
  ...
  if (blockedTables && blockedTables.length > 0) {
    const blockedFound = tables.filter(t =>
      blockedTables.some(blocked => t.includes(blocked.toLowerCase()))
    );
  ...
  if (allowedTables && allowedTables.length > 0) {
    const notAllowed = tables.filter(t =>
      !allowedTables.some(allowed => t.includes(allowed.toLowerCase()))
    );
